...
export interface CommonResponsesuccess<TData, TType = 'success' | 'info'>
  extends CommonResponseBase<TType> {
  success: true
  data: TData // Data is optional when success is true
}

export interface CommonResponseerror<TType = 'error'>
  extends CommonResponseBase<TType> {
  success: false
...
export type CommonResponse<TData, TType = 'error' | 'success' | 'info'> =
  | CommonResponsesuccess<TData, TType>
  | CommonResponseerror<TType>
...
export type ConditionalUser<T extends UserSubModels> = User & {
  [K in keyof T as T[K] extends true ? K : never]: K extends keyof FullUser
    ? FullUser[K]
    : never
}
